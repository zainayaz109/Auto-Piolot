...
def test_lanenet(image_path, weights_path, with_lane_fit=True):
    ...
    with sess.as_default():
        ...
        save_path = '/'.join(image_path.split('/')[:-1])
        plt.figure('mask_image')
        plt.savefig(save_path+'/mask_image.jpg')
        plt.imshow(mask_image[:, :, (2, 1, 0)])
        plt.savefig(save_path+'/mask_image.jpg')

        plt.figure('src_image')
        plt.imshow(image_vis[:, :, (2, 1, 0)])
        plt.savefig(save_path+'/src_image.jpg')

        plt.figure('instance_image')
        plt.imshow(embedding_image[:, :, (2, 1, 0)])
        plt.savefig(save_path+'/instance_image.jpg')

        plt.figure('binary_image')
        plt.imshow(binary_seg_image[0] * 255, cmap='gray')
        plt.savefig(save_path+'/binary_image.jpg')
