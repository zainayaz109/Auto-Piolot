...
train_dataset = hnet_data_processor.DataSet(glob.glob('/drive/MyDrive/new_projects/lane_detection/tusimple_lanemark_detection_Challenge /train_set/*.json'))
        ...
        for epoch in range(20005):
            ...
            if epoch % 1000 == 0:
                ...
                for k in range(len(trans_pts)):
                    cv2.circle(image[0], (int(trans_pts[0][k]), int(trans_pts[1][k])), 1, (0, 0, 255), 2)
                #warp_image = cv2.warpPerspective(image[0], R, dsize=(image[0].shape[1], image[0].shape[0]))
                cv2.imwrite("src.png", image[0])
                #cv2.imwrite("ret.png", warp_image)
            if epoch % 1000 == 0:
